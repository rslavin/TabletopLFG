...
class SiteAdminPublishers extends Component {
    ...
    render() {
        ...
        return (
            <div>
                <AddPublisher updateKey={this.updateKey.bind(this)} itemId={this.state.itemId}/>
                <PublisherList key={this.state.key} updateItemId={this.updateItemId.bind(this)}/>
            </div>
        );
    };
}
...
class AddPublisher extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = this.cleanPubFromState();
    }
    ...
    componentWillReceiveProps(newProps) {
        if (newProps.itemId != null && this.state.itemId != newProps.itemId) {
            $.ajax({
                url: constants.API_HOST + "/publisher/" + newProps.itemId,
                contentType: "application/json",
                cache: false,
                type: "GET",
            }).then(function (payload) {
                if (payload.hasOwnProperty('publisher')) {
                    this.setState(payload.publisher);
                }
    ...
    doCreate(e) {
        ...
        if (token != null) {
            var uri = "/publisher", method = "POST"; // create
            if (this.state.itemId != null) { // update
                uri = "/publisher/" + this.state.itemId;
                method = "PUT"
            }
            $.ajax({
                ...
                if (payload.hasOwnProperty('error')) {
                    ...
                    this.setState(this.cleanPubFromState());
                }
            }.bind(this), function (err) {
                console.log(err.responseText);
                                    ...
                                    <div className="form-group">
                                        ...
                                        <div className="col-md-4">
                                            <button id="singlebutton" name="singlebutton" disabled={this.state.loading}
                                                    ...
                                                    className="btn btn-block btn-primary">Add Publisher<SpinnerText
...
class PublisherList extends Component {
    ...
    render() {
        ...
        this.state.publishers.forEach(function (i) {
            itemRows.push({
                publisher: <a href="#" onClick={this.props.updateItemId.bind(this, i.id)}>{i.name}</a>,
            ...
            <div className="panel panel-primary">
                ...
                <div className="panel-body">
                    <Datatable
                        ...
                        initialSort={{prop: "publisher", isAscending: true}}
                    />
                </div>
            </div>
        );
...
class DeleteButton extends Component {
    ...
    onClick() {
        ...
        if (token != null) {
            ...
            $.ajax({
                url: constants.API_HOST + "/publisher/" + this.props.itemId,
                contentType: "application/json",
                cache: false,
                type: "DELETE",
                headers: {
    ...
    onClickUndelete() {
        ...
        if (token != null) {
            ...
            $.ajax({
                url: constants.API_HOST + "/publisher/" + this.props.itemId + "/undelete",
                contentType: "application/json",
                cache: false,
                type: "POST",
                headers: {
...
export default SiteAdminPublishers